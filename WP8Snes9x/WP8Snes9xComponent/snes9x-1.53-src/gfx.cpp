...
static void DisplayFrameRate (void)
{
	...
	sprintf_s(string, "%u fps", calcFps);
	S9xDisplayString(string, 2, IPPU.RenderedScreenWidth - (font_width - 1) * strlen(string) - 1, false);

#ifdef DEBUGGER
	const int	len = 8;
	sprintf_s(string, "%02d/%02d %02d", (int) IPPU.DisplayedRenderedFrameCount, (int) Memory.ROMFramesPerSecond, (int) IPPU.FrameCount);
#else
	const int	len = 5;
	sprintf_s(string, "%02d/%02d",      (int) IPPU.DisplayedRenderedFrameCount, (int) Memory.ROMFramesPerSecond);
#endif
	...
	for (int port = 0; port < 2; port++)
	{
		...
		switch (controller)
		{
			case CTL_MOUSE:
			{
				...
				uint8 buttons = buf[4];
				sprintf_s(string, "#%d %d: (%03d,%03d) %c%c", port, ids[0], x, y,
			...
			case CTL_SUPERSCOPE:
			{
				...
				int16 y = READ_WORD(p + 2);
				uint8 buttons = buf[4];
				sprintf_s(string, "#%d %d: (%03d,%03d) %c%c%c%c", port, ids[0], x, y,
			...
			case CTL_JUSTIFIER:
			{
				...
				sprintf_s(string, "#%d %d: (%03d,%03d) %c%c%c / (%03d,%03d) %c%c%c", port, ids[0],
			...
			case CTL_JOYPAD:
			{
				sprintf_s(string, "#%d %d:                  ", port, ids[0]);
			...
			case CTL_MP5:
			{
				for (int n = 0; n < 4; n++)
				{
					if (ids[n] != -1)
					{
						sprintf_s(string, "#%d %d:                  ", port, ids[n]);
...
static void DisplayWatchedAddresses (void)
{
	for (unsigned int i = 0; i < sizeof(watches) / sizeof(watches[0]); i++)
	{
		...
		if (watches[i].format == 1)
			sprintf_s(buf, "%s,%du = %u", watches[i].desc, watches[i].size, (unsigned int) displayNumber);
		else
		if (watches[i].format == 3)
			sprintf_s(buf, "%s,%dx = %X", watches[i].desc, watches[i].size, (unsigned int) displayNumber);
		else // signed
		{
			if (watches[i].size == 1)
			...
			sprintf_s(buf, "%s,%ds = %d", watches[i].desc, watches[i].size, (int) displayNumber);
		}
