...
uint32 CMemory::FileLoader (uint8 *buffer, const char *filename, int32 maxsize)
{
	...
	_splitpath_s(filename, drive, dir, name, exts);
	_makepath_s(fname, drive, dir, name, exts);
	...
	switch (nFormat)
	{
		...
		{
			STREAM	fp;
			OPEN_STREAM(&fp, fname, "rb");
			if (!fp)
				return (0);

			strcpy_s(ROMFilename, fname);
...
again:
	...
	{
		strncpy_s(LastRomFilename, filename, PATH_MAX + 1);
...
bool8 CMemory::LoadROM (unsigned char *rom, size_t length)
{
	int	retry_count = 0;
...
again:
	...
	int32 totalFileSize = length;

	//totalFileSize = FileLoader(ROM, filename, MAX_ROM_SIZE);
	totalFileSize = HeaderRemove(totalFileSize, HeaderCount, rom);
	if (!totalFileSize)
		return (FALSE);

	for (int i = 0; i < totalFileSize && i < MAX_ROM_SIZE; i++)
	{
		ROM[i] = rom[i];
	}
...
bool8 CMemory::LoadSufamiTurbo (const char *cartA, const char *cartB)
{
	...
	if (Multi.cartSizeA)
	{
		...
		strcpy_s(Multi.fileNameA, cartA);
	...
	if (Multi.cartSizeB)
	{
		...
		strcpy_s(Multi.fileNameB, cartB);
	...
	strcpy_s(path, S9xGetDirectory(BIOS_DIR));
	strcat_s(path, SLASH_STR);
	strcat_s(path, "STBIOS.bin");
	...
	if (Multi.cartSizeA)
		strcpy_s(ROMFilename, Multi.fileNameA);
	else
	if (Multi.cartSizeB)
		strcpy_s(ROMFilename, Multi.fileNameB);
	else
		strcpy_s(ROMFilename, path);
...
bool8 CMemory::LoadSRTC (void)
{
	...
	fopen_s(&fp, S9xGetFilename(".rtc", SRAM_DIR), "rb");
	if (!fp)
		return (FALSE);
...
bool8 CMemory::SaveSRTC (void)
{
	...
	fopen_s(&fp, S9xGetFilename(".rtc", SRAM_DIR), "wb");
	if (!fp)
		return (FALSE);
...
bool8 CMemory::LoadSRAM (const char *filename)
{
	...
	if (Multi.cartType && Multi.sramSizeB)
	{
		...
		strcpy_s(temp, ROMFilename);
		strcpy_s(ROMFilename, Multi.fileNameB);

		size = (1 << (Multi.sramSizeB + 3)) * 128;

		fopen_s(&file, S9xGetFilename(".srm", SRAM_DIR), "rb");
		if (file)
		{
	...
	if (size)
	{
		...
		if (Settings.BS && !Settings.BSXItself)
		{
			...
			strcpy_s(path, S9xGetDirectory(SRAM_DIR));
			strcat_s(path, SLASH_STR);
			strcat_s(path, "BS-X.srm");
...
bool8 CMemory::SaveSRAM (const char *filename)
{
	...
	if (Multi.cartType && Multi.sramSizeB)
	{
		...
		strcpy_s(temp, ROMFilename);
		strcpy_s(ROMFilename, Multi.fileNameB);
		strcpy_s(name, S9xGetFilename(".srm", SRAM_DIR));
...
void CMemory::ParseSNESHeader (uint8 *RomHeader)
{
	...
	strncpy_s(ROMName, (char *) &RomHeader[0x10], ROM_NAME_LEN - 1);
	if (bs)
...
void CMemory::InitROM (void)
{
	...
	strcpy_s(RawROMName, ROMName);
	sprintf_s(displayName, "%s", SafeANK(ROMName));
	sprintf_s(ROMName, "%s", Safe(ROMName));
	sprintf_s(ROMId, "%s", Safe(ROMId));

	sprintf_s(String, "\"%s\" [%s] %s, %s, %s, %s, SRAM:%s, ID:%s, CRC32:%08X",
...
const char * CMemory::StaticRAMSize (void)
{
	...
	if (SRAMSize > 16)
		strcpy_s(str, "Corrupt");
	else
		sprintf_s(str, "%dKbits", 8 * (SRAMMask + 1) / 1024);
...
const char * CMemory::Size (void)
{
	...
	if (ROMSize < 7 || ROMSize - 7 > 23)
		...
		sprintf_s(str, "%dMbits", 1 << (ROMSize - 7));
...
const char * CMemory::Revision (void)
{
	...
	sprintf_s(str, "1.%d", HiROM ? ((ExtendedFormat != NOPE) ? ROM[0x40ffdb] : ROM[0xffdb]) : ROM[0x7fdb]);
...
const char * CMemory::KartContents (void)
{
	...
	if (Settings.SPC7110RTC)
		strcpy_s(chip, "+SPC7110+RTC");
	else
	if (Settings.SPC7110)
		strcpy_s(chip, "+SPC7110");
	else
	if (Settings.SRTC)
		strcpy_s(chip, "+S-RTC");
	else
	...
	if (Settings.SETA == ST_010)
		strcpy_s(chip, "+ST-010");
	else
	if (Settings.SETA == ST_011)
		strcpy_s(chip, "+ST-011");
	else
	if (Settings.SETA == ST_018)
		strcpy_s(chip, "+ST-018");
	else
	if (Settings.DSP)
		sprintf_s(chip, "+DSP-%d", Settings.DSP);
	else
		strcpy_s(chip, "");

	sprintf_s(str, "%s%s", contents[(ROMType & 0xf) % 3], chip);
...
void CMemory::MakeRomInfoText (char *romtext)
{
	...
	sprintf_s(temp,   "            Cart Name: %s", ROMName);
	strcat_s(romtext, 256, temp);
	sprintf_s(temp, "\n            Game Code: %s", ROMId);
	strcat_s(romtext, 256, temp);
...
void CMemory::CheckForAnyPatch (const char *rom_filename, bool8 header, int32 &rom_size)
{
	...
	_splitpath_s(rom_filename, drive, dir, name, ext);
	_makepath_s(fname, drive, dir, name, "ups");

	OPEN_STREAM(&patch_file, fname, "rb");
	if (patch_file != NULL)
	{
	...
	_splitpath_s(rom_filename, drive, dir, name, ext);
	_makepath_s(fname, drive, dir, name, "ips");
