...
void S9xReportControllers (void)
{
	...
	{
		c += sprintf_s(c, 128, "Port %d: ", port + 1);

		switch (newcontrollers[port])
		{
			case NONE:
				c += sprintf_s(c, 128, "<none>. ");
				break;

			case MP5:
				c += sprintf_s(c, 128, "MP5 with pads");
				for (int i = 0; i < 4; i++)
				{
					if (mp5[port].pads[i] == NONE)
						c += sprintf_s(c, 128, " <none>. ");
					else
						c += sprintf_s(c, 128, " #%d. ", mp5[port].pads[i] + 1 - JOYPAD0);
				}
			...
			case JOYPAD6:
			case JOYPAD7:
				c += sprintf_s(c, 128, "Pad #%d. ", (int) (newcontrollers[port] - JOYPAD0 + 1));
				break;

			case MOUSE0:
			case MOUSE1:
				c += sprintf_s(c, 128, "Mouse #%d. ", (int) (newcontrollers[port] - MOUSE0 + 1));
				break;

			case SUPERSCOPE:
				if (port == 0)
					c += sprintf_s(c, 128, "Superscope (cannot fire). ");
				else
					c += sprintf_s(c, 128, "Superscope. ");
				break;

			case ONE_JUSTIFIER:
				if (port == 0)
					c += sprintf_s(c, 128, "Blue Justifier (cannot fire). ");
				else
					c += sprintf_s(c, 128, "Blue Justifier. ");
				break;

			case TWO_JUSTIFIERS:
				if (port == 0)
					c += sprintf_s(c, 128, "Blue and Pink Justifiers (cannot fire). ");
				else
					c += sprintf_s(c, 128, "Blue and Pink Justifiers. ");
				break;
		}
	}
...
char * S9xGetCommandName (s9xcommand_t command)
{
	...
	switch (command.type)
	{
		...
		case S9xButtonCommand:
			...
			return (_strdup(command_names[command.button.command]));
		...
		case S9xAxisPort:
		case S9xPointerPort:
			return (_strdup("BUG: Port should have handled this instead of calling S9xGetCommandName()"));
		...
		default:
			return (_strdup("BUG: Unknown command type"));
	}
			...
			else
			{
				switch ((enum command_numbers) (i = cmd.button.command))
				{
					...
					case IncFrameRate:
						...
						{
							sprintf_s(buf, "Frame skip: %d", Settings.SkipFrames - 1);
							S9xSetInfoString(buf);
						}
					...
					case IncEmuTurbo:
						...
						sprintf_s(buf, "Turbo frame skip: %d", Settings.TurboSkipFrames);
						S9xSetInfoString(buf);
						break;
					...
					case IncFrameTime: // Increase emulated frame time by 1ms
						...
						sprintf_s(buf, "Emulated frame time: %dms", Settings.FrameTime / 1000);
						S9xSetInfoString(buf);
						break;
					...
					case IncTurboSpeed:
						if (turbo_time >= 120)
							break;
						turbo_time++;
						sprintf_s(buf, "Turbo speed: %d", turbo_time);
						S9xSetInfoString(buf);
						break;
					...
					case QuickLoad010:
					{
						...
						if (S9xUnfreezeGame(filename))
						{
							sprintf_s(buf, "%s.%03d loaded", def, i - QuickLoad000);
							S9xSetInfoString(buf);
						}
						else
					...
					case QuickSave010:
					{
						...
						sprintf_s(buf, "%s.%03d saved", def, i - QuickSave000);
						S9xSetInfoString(buf);
					...
					case SoundChannel7:
						...
						sprintf_s(buf, "Sound channel %d toggled", i - SoundChannel0);
						S9xSetInfoString(buf);
						break;
					...
					case SwapJoypads:
						...
						strcpy_s(buf, "Swap pads: P1=");
						i = 14;
						if (newcontrollers[0] == NONE)
						{
							strcpy_s(buf + i, 256 - i, "<none>");
							i += 6;
						}
						else
						{
							sprintf_s(buf + i, 256 - i, "Joypad%d", newcontrollers[0] - JOYPAD0 + 1);
							i += 7;
						}

						strcpy_s(buf + i, 256 - i, " P2=");
						i += 4;
						if (newcontrollers[1] == NONE)
							strcpy_s(buf + i, 256 - i, "<none>");
						else
							sprintf_s(buf + i, 256 - i, "Joypad%d", newcontrollers[1] - JOYPAD0 + 1);
					...
					case SeekToFrame:
						if (S9xMovieActive())
						{
							sprintf_s(buf, "Select frame number (current: %d)", S9xMovieGetFrameCounter());
