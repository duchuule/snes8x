...
const char * S9xProActionReplayToRaw (const char *code, uint32 &address, uint8 &byte)
{
	...
	if (strlen(code) != 8 || !S9xAllHex(code, 8) || sscanf_s(code, "%x", &data) != 1)
...
const char * S9xGoldFingerToRaw (const char *code, uint32 &address, bool8 &sram, uint8 &num_bytes, uint8 bytes[3])
{
	...
	strncpy_s(tmp, code, 5);
	tmp[5] = 0;
	if (sscanf_s(tmp, "%x", &address) != 1)
	...
	for (i = 0; i < 3; i++)
	{
		...
		strncpy_s(tmp, code + 5 + i * 2, 2);
		tmp[2] = 0;
		if (sscanf_s(tmp, "%x", &byte) != 1)
			break;
		bytes[i] = (uint8) byte;
	}
...
const char * S9xGameGenieToRaw (const char *code, uint32 &address, uint8 &byte)
{
	...
	strcpy_s(new_code, "0x");
	strncpy_s(new_code + 2, 12, code, 4);
	strcpy_s(new_code + 6, 12, code + 5);
